...
def run_analysis(algorithm_func, algo_name, runs=100):
    ...
    k = forge.NearestNeighbourIndex(robot)

    closest = k.query(robot.destination, 10)

    for _ in tqdm(range(runs), desc=f"Running {algo_name}"):
        closest = k.query(robot.destination, 10)[0]
        c_len = len(closest)
        gen_pop = forge.generateChromosome(pop-c_len, robot.dof)
        if c_len > 0:
            gen_pop += closest
        output = forge.socialGroupOptimization(pop, itr, gen_pop, robot, True)
        end_effector = forge.forward_kinematics(output.best_gene, robot)[-1]
        k.insert(end_effector, output.best_gene)
